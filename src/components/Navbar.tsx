...
export default function Navbar() {
  ...
  useEffect(() => {
      ...
      if (session?.user) {
        fetch('/api/messages')
          ...
          .then(data => {
            ...
            // Mesaj sayısını hesapla ama kullanma (şu an için gerekli değil)
            data.filter((msg: Message) =>
              msg && msg.receiver && msg.receiver.id === myId && msg.isRead === false
            );
          })
          .catch(() => {
    ...
    updateUnread();
    window.addEventListener('unread-messages-updated', updateUnread)
    return () => window.removeEventListener('unread-messages-updated', updateUnread)
  }, [session])
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex h-16 justify-between">
              ...
              <div className="hidden sm:ml-6 sm:flex sm:items-center">
                ...
                <Link
                  href="/orders"
                  className="ml-4 p-2 text-sm font-medium text-secondary hover:text-primary"
                >
                  Siparişlerim
                </Link>
                {session ? (
                  ...
                  <button
                    onClick={() => {
                      const shouldLogin = confirm('Mesajları görüntülemek için giriş yapmanız gerekiyor. Giriş sayfasına yönlendirilsiniz mi?');
                      if (shouldLogin) {
                        window.location.href = '/login';
                      }
                    }}
                    className="ml-4 p-2 text-sm font-medium text-secondary hover:text-primary cursor-pointer"
                  >
                    <span>Mesajlar</span>
                  </button>
                )}
            ...
            <div className="space-y-1 pb-3 pt-2">
              ...
              {session ? (
                ...
                <Disclosure.Button
                  ...
                  className="block w-full px-3 py-2 text-base font-medium text-secondary hover:bg-input hover:text-secondary-dark text-left"
                >
                  Mesajlar
                </Disclosure.Button>
              )}
